...
# /mnt/raw/DeviceLogs/{COM3,CT}/{CS3M2129401008,CT4F1931001007,CS4M2017701037,CS4M2115901013,CS3M2208001010,CS4M2209401030,CS3M2129201020,CS3M2206601012,CS4M2115901013}/*/*/*/
...
dbutils.widgets.text('sourceFilePath','/mnt/raw/DeviceLogs/{COM3,CT}/CS3M2031601005/*/*/{ML,MLT}/')
...
df_Source = (spark.read.option('quote','{')
...
#     df_ueLogs_CNT.display()
    ...
    try:
        ...
        dfcnt = df_Source_ULLogs.filter(col('HdrDataContractNbr').isin(lst)).distinct().count()
        if (dfcnt == 0):
            return
        ...
        df_AllCycles = (df_AllCycles_Decrypted.withColumn("AlleIDDecryptedFlg", when(col('BDIDEncryptdFlg') == "True","True")
...
def ProcessULLogs84005_84006(df_Source_ULLogsCT,ulLogs_ConfigId,allLogs_ConfigId):
    ...
    try:
        ...
        dfcnt = df_Source_ULLogsCT.filter(col('HdrDataContractNbr').isin(lst)).distinct().count()
        # if (dfcnt == 0):
        #     return
...
def upsertToDelta(microBatchOutputDF, batchId):
    ...
    df_logSource_FileNameUUID = df_logSource.join(broadcast(df_logSourceFileProcessed),'SourceFilePath','inner')   
    
    df_logSource_FileNameUUID.cache()
    try:
        print(batchId)
        print(df_logSource_FileNameUUID.select('FileNameUUID').drop_duplicates().count())
...
# print(batchId)
print(df_logSource_FileNameUUID.select('SourceFilePath').drop_duplicates().count())
startTime = datetime.now()
print("Current Time =", startTime)        
...
df_Source_ULLogsCT = df_LogsULCT
